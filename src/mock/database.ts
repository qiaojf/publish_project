...
function initialDatabase(): Database {
  return {
    users: [
      { id: 1, username: 'admin', password: 'admin123', name: '系统管理员', role: 'admin', status: 'active', created_at: ago(24 * 120), updated_at: ago(24 * 120) },
      { id: 2, username: 'employee', password: 'employee123', name: '林知夏', role: 'employee', status: 'active', created_at: ago(24 * 70), updated_at: ago(24 * 70) },
    ...
    targets: [
      { id: 1, name: '内部页面', content_types: ['html', 'dynamic'], publish_root: '/data/company/site/', base_url: 'https://internal.example.com/site/', enabled: true, created_at: ago(24 * 90) },
      { id: 2, name: 'PPT 发布区', content_types: ['ppt'], publish_root: '/data/company/presentation/', base_url: 'https://internal.example.com/presentation/', enabled: true, created_at: ago(24 * 80) },
      { id: 3, name: '文档中心', content_types: ['pdf', 'word', 'excel'], publish_root: '/var/www/company/docs/', base_url: 'https://internal.example.com/docs/', enabled: true, created_at: ago(24 * 65) },
      { id: 4, name: '公共文件', content_types: ['image', 'file'], publish_root: '/data/company/assets/', base_url: 'https://internal.example.com/assets/', enabled: true, created_at: ago(24 * 40) }
    ],
    contents: [
      { id: 101, title: '2026 秋季产品路线图', description: '面向销售与交付团队的产品方向说明与里程碑。', category: '产品资料', content_type: 'ppt', file_name: 'product-roadmap-2026.pptx', file_size: 8_421_300, content_body: null, created_by: 2, creator_name: '林知夏', created_at: ago(62), updated_at: ago(4), submitted_at: ago(4), review_status: 'pending', publish_status: 'unpublished', publish_target_id: 2, publish_target_name: 'PPT 发布区', published_at: null, view_url: null, reject_reason: null, failure_reason: null },
      { id: 102, title: '品牌视觉资产使用规范', description: 'Logo、安全区、字体与常用物料规范。', category: '品牌素材', content_type: 'pdf', file_name: 'brand-guideline-v3.pdf', file_size: 4_109_120, created_by: 2, creator_name: '林知夏', created_at: ago(120), updated_at: ago(29), review_status: 'rejected', publish_status: 'unpublished', publish_target_id: 3, publish_target_name: '文档中心', reject_reason: '请补充深色背景下的 Logo 使用示例，并更新文档版本号。' },
      { id: 103, title: '新员工信息安全培训', description: '入职必修的信息安全培训材料。', category: '培训材料', content_type: 'ppt', file_name: 'security-onboarding.pptx', file_size: 11_480_200, created_by: 2, creator_name: '林知夏', created_at: ago(350), updated_at: ago(190), submitted_at: ago(200), review_status: 'approved', publish_status: 'published', publish_target_id: 2, publish_target_name: 'PPT 发布区', published_at: ago(190), view_url: 'https://internal.example.com/presentation/security-onboarding/' },
      { id: 104, title: '客户案例素材包', description: '制造业客户案例图片与可公开使用素材。', category: '销售方案', content_type: 'file', file_name: 'case-assets.zip', file_size: 22_309_500, created_by: 2, creator_name: '林知夏', created_at: ago(14), updated_at: ago(12), review_status: 'draft', publish_status: 'unpublished', publish_target_id: 4, publish_target_name: '公共文件' },
      { id: 105, title: '差旅费用报销制度', description: '适用于全体员工的差旅申请与费用报销标准。', category: '制度规范', content_type: 'pdf', file_name: 'travel-policy-2026.pdf', file_size: 1_804_300, created_by: 3, creator_name: '王启', created_at: ago(420), updated_at: ago(260), submitted_at: ago(270), review_status: 'approved', publish_status: 'published', publish_target_id: 3, publish_target_name: '文档中心', published_at: ago(260), view_url: 'https://internal.example.com/docs/travel-policy-2026/' },
      { id: 106, title: '区域销售数据模板', description: '区域团队月度销售数据填报模板。', category: '销售方案', content_type: 'excel', file_name: 'regional-sales.xlsx', file_size: 784_300, created_by: 3, creator_name: '王启', created_at: ago(180), updated_at: ago(90), submitted_at: ago(94), review_status: 'approved', publish_status: 'failed', publish_target_id: 3, publish_target_name: '文档中心', failure_reason: '转换服务暂时无法读取工作簿中的外部链接。' },
      { id: 107, title: '研发环境访问指南', description: '开发环境、测试环境和常见权限申请入口。', category: '制度规范', content_type: 'html', file_name: 'dev-access.html', file_size: 43_800, created_by: 1, creator_name: '系统管理员', created_at: ago(58), updated_at: ago(38), review_status: 'approved', publish_status: 'published', publish_target_id: 1, publish_target_name: '内部页面', published_at: ago(38), view_url: 'https://internal.example.com/site/dev-access/' },
      { id: 108, title: '月度运营播报', description: '八月重点运营数据与九月行动安排。', category: '产品资料', content_type: 'ppt', file_name: 'monthly-operations.pptx', file_size: 6_340_000, created_by: 3, creator_name: '王启', created_at: ago(31), updated_at: ago(7), submitted_at: ago(7), review_status: 'pending', publish_status: 'unpublished', publish_target_id: 2, publish_target_name: 'PPT 发布区' }
    ],
    reviews: [
    ...
    publishLogs: [
      { id: 1, created_at: ago(38), content_id: 107, content_title: '研发环境访问指南', content_type: 'html', target_name: '内部页面', status: 'success', view_url: 'https://internal.example.com/site/dev-access/' },
      { id: 2, created_at: ago(90), content_id: 106, content_title: '区域销售数据模板', content_type: 'excel', target_name: '文档中心', status: 'failed', failure_reason: '转换服务暂时无法读取工作簿中的外部链接。' },
      { id: 3, created_at: ago(190), content_id: 103, content_title: '新员工信息安全培训', content_type: 'ppt', target_name: 'PPT 发布区', status: 'success', view_url: 'https://internal.example.com/presentation/security-onboarding/' },
      { id: 4, created_at: ago(260), content_id: 105, content_title: '差旅费用报销制度', content_type: 'pdf', target_name: '文档中心', status: 'success', view_url: 'https://internal.example.com/docs/travel-policy-2026/' }
    ]
  }
}
...
function findContent(db: Database, id: number): ContentItem {
  ...
  const user = activeUser(db)
  if (user.role !== 'admin' && item.created_by !== user.id && item.publish_status !== 'published') throw new Error('无权查看该内容')
  return item
}
...
export const mockDb = {
  ...
  async getUsers(query: UserQuery) {
    ...
    if (query.keyword) items = items.filter((item) => `${item.username}${item.name}`.toLowerCase().includes(query.keyword!.toLowerCase()))
  ...
  async createUser(payload: UserPayload) {
    ...
    const user: StoredUser = { ...payload, password: payload.password || 'demo123', id: nextId(db.users), created_at: now(), updated_at: now() }
  ...
  async updateUser(id: number, payload: UserPayload) {
    ...
    Object.assign(user, payload, { password: payload.password || user.password }); recordOperation(db, '编辑用户', `用户 #${id}`, `更新用户 ${user.name}`); save(db)
  ...
  async createContent(payload: ContentPayload) {
    ...
    const item: ContentItem = { id: nextId(db.contents), title: payload.title, description: payload.description, category: payload.category, content_type: payload.content_type, content_body: payload.content_body, file_name: payload.file?.name || payload.file_name, file_size: payload.file?.size || payload.file_size, created_by: user.id, creator_name: user.name, created_at: now(), updated_at: now(), review_status: 'draft', publish_status: 'unpublished', publish_target_id: payload.publish_target_id, publish_target_name: targetName(db, payload.publish_target_id) }
  ...
  async updateContent(id: number, payload: ContentPayload) {
    ...
    Object.assign(item, { title: payload.title, description: payload.description, category: payload.category, content_type: payload.content_type, content_body: payload.content_body, file_name: payload.file?.name || payload.file_name || item.file_name, file_size: payload.file?.size || payload.file_size || item.file_size, publish_target_id: payload.publish_target_id, publish_target_name: targetName(db, payload.publish_target_id), updated_at: now() })
  ...
  async getTargets(): Promise<PublishTarget[]> {
    await wait(); const db = load(); const user = activeUser(db)
    return db.targets.filter((item) => user.role === 'admin' || item.enabled).map((item) => user.role === 'admin' ? { ...item } : { id: item.id, name: item.name, content_types: item.content_types, enabled: item.enabled, created_at: item.created_at })
  },
  ...
  async search(query: ContentQuery & { date_from?: string; date_to?: string }) { await wait(320); const db = load(); activeUser(db); let items = db.contents.filter((item) => item.publish_status === 'published'); if (query.keyword) items = items.filter((item) => `${item.title}${item.description}`.toLowerCase().includes(query.keyword!.toLowerCase())); if (query.content_type) items = items.filter((item) => item.content_type === query.content_type); if (query.category) items = items.filter((item) => item.category === query.category); if (query.date_from || query.date_to) items = items.filter((item) => dateInRange(item.published_at || '', query.date_from, query.date_to)); return paginate(items.sort((a, b) => (b.published_at || '').localeCompare(a.published_at || '')), query.page, query.page_size) },
