...
import type { PublishTarget, PublishTargetType } from '@/types/publish'
...
const targetTypeLabels: Record<PublishTargetType, string> = { local: '公司服务器目录', sftp: 'SFTP', github: 'GitHub Repository', github_pages: 'GitHub Pages', onedrive: 'Microsoft OneDrive', dropbox: 'Dropbox' }
function targetLocation(target?: PublishTarget | null) { if (!target) return '—'; if (target.target_type === 'local') return target.publish_root || '—'; if (target.target_type === 'sftp') return `${target.config?.host || '—'}:${target.config?.port || 22}${target.config?.remote_root || ''}`; if (target.target_type === 'github' || target.target_type === 'github_pages') return `${target.config?.owner || '—'}/${target.config?.repo || '—'} @ ${target.config?.branch || '—'} / ${target.config?.repo_path || ''}`; if (target.target_type === 'onedrive') return `Drive ${target.config?.drive_id || '—'} / ${target.config?.folder_path || ''}`; return String(target.config?.folder_path || '—') }
function targetUrl(target?: PublishTarget | null) { return target?.base_url || String(target?.config?.base_url || '—') }
  ...
  <div class="page-shell" v-loading="loading">
    ...
    <div v-if="detail" class="review-grid">
      ...
      <aside class="stack-12">
        <section class="paper-card review-section target-card"><div class="section-head"><span class="section-kicker">DESTINATION</span><h2>发布目标核对</h2></div><dl><div><dt>目标名称</dt><dd>{{ detail.publish_target?.name }}</dd></div><div><dt>目标类型</dt><dd>{{ detail.publish_target ? targetTypeLabels[detail.publish_target.target_type] : '—' }}</dd></div><div><dt>适用类型</dt><dd>{{ detail.publish_target?.content_types.map(item => CONTENT_TYPES[item]).join('、') }}</dd></div><div><dt>目标位置</dt><dd class="mono path">{{ targetLocation(detail.publish_target) }}</dd></div><div><dt>访问 URL</dt><dd class="mono path">{{ targetUrl(detail.publish_target) }}</dd></div><div v-if="detail.publish_target?.credential_ref"><dt>凭证引用</dt><dd class="mono path">{{ detail.publish_target.credential_ref }}</dd></div><div><dt>当前状态</dt><dd><el-tag :type="detail.publish_target?.enabled ? 'success' : 'danger'">{{ detail.publish_target?.enabled ? '启用' : '禁用' }}</el-tag></dd></div></dl></section>
