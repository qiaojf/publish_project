...
async function validate() { if (!formRef.value) return false; const ok = await formRef.value.validate()?.catch(() => false); if (!ok) return false; const model = formRef.value.model; if (!['html', 'dynamic'].includes(model.content_type) && !model.file && !initial.value?.file_name) { ElMessage.warning('请选择内容文件'); return false } return true }
