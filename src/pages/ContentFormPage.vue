...
onMounted(async () => { loading.value = true; try { const [loadedTargets] = await Promise.all([getPublishTargets(), categories.load()]); targets.value = loadedTargets; if (contentId.value) { initial.value = await getContent(contentId.value); if (!auth.isAdmin && (initial.value.created_by !== auth.user?.id || initial.value.review_status === 'pending')) { ElMessage.error('当前内容不可编辑'); router.replace(`/contents/${contentId.value}`) } } } catch (e) { ElMessage.error(e instanceof Error ? e.message : '表单加载失败') } finally { loading.value = false } })
...
<template>
  <div class="page-shell" v-loading="loading">
    ...
    <section class="paper-card editor-card"><div class="editor-head"><span class="step-no">01</span><div><h2>内容资料</h2><p>带 * 的字段为必填项</p></div></div><ContentForm v-if="!loading" ref="formRef" :initial="initial" :targets="targets" :categories="categories.activeNames" /></section>
