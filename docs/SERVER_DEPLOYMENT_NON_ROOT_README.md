...
PUBLISH_CREDENTIAL_INSTAGRAM_COMPANY_ACCESS_TOKEN=替换为Instagram User access token
```

Instagram Reels 还要求公网 HTTPS 域名能够访问 `/local-published/_instagram/`；用户级 Caddy 已提供该目录，需由主机管理员把正式 HTTPS 域名反向代理到用户端口。完整步骤见 [`INSTAGRAM_PUBLISHING_SETUP.md`](INSTAGRAM_PUBLISHING_SETUP.md)。
